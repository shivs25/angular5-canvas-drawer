...
exports.elementsReducer = function (state, action) {
    ...
    switch (action.type) {
        ...
        case actions_1.REMOVE_OBJECTS: {
            return Object.assign({}, state, {
                elements: state.elements.filter(function (t) { return action.itemIds.indexOf(t.id) < 0; })
            });
        }
        ...
        case actions_1.UNGROUP_OBJECT: {
            ...
            var index = state.elements.indexOf(item);
            return Object.assign({}, state, {
                elements: state.elements.slice(0, index).concat(action.item.objects.map(function (x) { return utilities_1.cloneDeep(x); }), state.elements.slice(index + 1))
            });
        }
