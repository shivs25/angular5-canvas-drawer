...
var DataStoreService = /** @class */ (function () {
    ...
    DataStoreService.prototype.moveObjectsDown = function (items) {
        ...
        if (min >= 0) {
            var idsToExclude_1 = indices.map(function (x) { return x.id; });
            var elements = this.elements.slice(0, min).filter(function (x) { return idsToExclude_1.indexOf(x.id) < 0; }).concat(indices.sort(function (a, b) { return a.index - b.index; }).map(function (x) { return x.item; }), this.elements.slice(min).filter(function (x) { return idsToExclude_1.indexOf(x.id) < 0; }));
            this._ngRedux.dispatch({
    ...
    DataStoreService.prototype.moveObjectsUp = function (items) {
        ...
        if (max < this.elements.length) {
            var idsToExclude_2 = indices.map(function (x) { return x.id; });
            var elements = this.elements.slice(0, max + 1).filter(function (x) { return idsToExclude_2.indexOf(x.id) < 0; }).concat(indices.sort(function (a, b) { return a.index - b.index; }).map(function (x) { return x.item; }), this.elements.slice(max + 1).filter(function (x) { return idsToExclude_2.indexOf(x.id) < 0; }));
            this._ngRedux.dispatch({
                type: actions_1.CHANGE_Z_INDEX,
                elements: elements
            });
